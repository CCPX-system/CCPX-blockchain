...
// ============================================================================================================================
// Invoke - Our entry point for Invocations程序入口
// ============================================================================================================================
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	fmt.Println("invoke is running " + function)

	// Handle different functions
	if function == "init" {													//initialize the chaincode state, used as reset初始化链码状态，用作复位
		return t.Init(stub, "init", args)
	} else if function == "delete" {										//deletes an entity from its state从状态删除实体
		res, err := t.Delete(stub, args)
		cleanTrades(stub)													//lets make sure all open trades are still valid确认交易有效
		return res, err
	} else if function == "write" {											//writes a value to the chaincode state向链代码状态写值
		...
		return res, err
	} else if function == "open_trade" {									//create a new trade order创建新交易请求
		return t.open_trade(stub, args)
	} else if function == "perform_trade" {									//forfill an open trade order实现开放交易秩序
		res, err := t.perform_trade(stub, args)
		cleanTrades(stub)													//lets clean just in case清空
		return res, err
	} else if function == "remove_trade" {									//cancel an open trade order取消未平仓交易订单
...
func (t *SimpleChaincode) Query(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "read" {													//read a variable
		fmt.Println("300 enquiry successfully")	
		return t.read(stub, args)
	}
	fmt.Println("400 have no permission to enquiry" + function)						//error
...
func (t *SimpleChaincode) Write(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	if len(args) != 2 {
		return nil, errors.New("200-have no permission to record Incorrect number of arguments. Expecting 2. name of the variable and value to set")
	}
	...
	fmt.Println("100-record successfully")
	return nil, nil
}
...
func (t *SimpleChaincode) init_marble(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	// "asdf", "blue", "35", "bob"
	if len(args) != 4 {
		return nil, errors.New("600-Init Error Incorrect number of arguments. Expecting 4")
	}

	//input sanitation
	fmt.Println("- start init marble")
	if len(args[0]) <= 0 {
		return nil, errors.New("600-Init Error 1st argument must be a non-empty string")
	}
	if len(args[1]) <= 0 {
		return nil, errors.New("600-Init Error 2nd argument must be a non-empty string")
	}
	if len(args[2]) <= 0 {
		return nil, errors.New("600-Init Error 3rd argument must be a non-empty string")
	}
	if len(args[3]) <= 0 {
		return nil, errors.New("600-Init Error 4th argument must be a non-empty string")
	}
	name := args[0]
	...
	if err != nil {
		return nil, errors.New("600-Init Error 3rd argument must be a numeric string")
	}

	//check if marble already exists
	marbleAsBytes, err := stub.GetState(name)
	if err != nil {
		return nil, errors.New("600-Init Error Failed to get marble name")
	}
	res := Marble{}
	json.Unmarshal(marbleAsBytes, &res)
	if res.Name == name{
		fmt.Println("600-Init Error This marble arleady exists: " + name)
		fmt.Println(res);
		return nil, errors.New("600-Init Error This marble arleady exists")				//all stop a marble by this name exists
	}
	...
	marblesAsBytes, err := stub.GetState(marbleIndexStr)
	if err != nil {
		return nil, errors.New("600-Init Error Failed to get marble index")
	}
	var marbleIndex []string
	json.Unmarshal(marblesAsBytes, &marbleIndex)							//un stringify it aka JSON.parse()
...
// ============================================================================================================================
func (t *SimpleChaincode) perform_trade(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {//执行交易
	var err error
